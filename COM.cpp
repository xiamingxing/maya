...
void COM::CheckMessageTime() //公告
{
	...
	if(messageTime.wSecond%10==0 )//电光板
	{
...
void COM::CheckPKTime()		//PK竞赛
{
	...
	if(guildTime.wHour == 19 && guildTime.wMinute>= 55)	// 19点45进行公告!
	{
		...
		{
			strMsg.Format( "单人格斗大赛还有 %d 分钟将会开放进入,20:05时将会关闭,请大家准备哦!", nTime);//"恶魔广场还有 %d 分钟将会开放,请大家准备就绪!"
			Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
		}
	}
	...
	if(guildTime.wHour >= 20 && guildTime.wMinute > 5 && PKover == FALSE)
	{
		...
		{ 
			...
			/*if( PKnum < 4 )
			{
				strMsg.Format("当前人数只有:[ %d ]个，无法进行大赛.",PKnum);
				Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
				pUser->ZoneMoveReq(1 ,1301,267);
			}*/
	...
	if( PKover == TRUE)
	{
		...
		{
				strMsg.Format("玩家[ %s ]获得了最终胜利.成为本届PK王!!!",winName);
				Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			PKover = FALSE;
		}
	}
...
void COM::CheckPKShaRenTime()		//PK竞赛
{
	...
	if(guildTime.wHour == 20 && guildTime.wMinute>= 15 && guildTime.wMinute< 20)	// 20点15分开始公告
	{
		...
		{
			//strMsg.Format( "为了大家踊跃参加大赛,每杀一个玩家奖励10个标志,封顶值为200个,也就是杀20个人以下都奖励.");
			//Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
		}
		else if(guildTime.wSecond == 50)			
		{
			//strMsg.Format( "杀人狂大赛的进入地点是乍旦NPC处选择进入");
			//Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
		}
	}
...
void COM::CheckDevilTime()		//恶魔广场公告
{
	...
	int nTime = 0;
	CString strMsg;
                 if(guildTime.wDayOfWeek == 5)//每星期六开放
					{
						if(guildTime.wHour == 18 && guildTime.wMinute>= 45)	// 18点45进行公告!
						{
						...
						if(guildTime.wHour  == 19 && guildTime.wMinute <= 30)
						{
								nTime = 30 - guildTime.wMinute;
								if(nTime <=5 && guildTime.wSecond == 0)
								{
									strMsg.Format( "恶魔广场在 %d 分钟后关闭! 下周六晚上19:00点恶魔广场继续开放！", nTime);//"恶魔广场还有 %d 分钟将会关闭!"
									Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
								}
						}
					}
