...
BOOL CServerDlg::GetLoadYeHuoINI()
{
	...
	szfilename += _T("\\server.ini");	//yehuo.ini
	CString tttmp;
		...
		GetPrivateProfileString("ÉèÖÃ", "Ê¹ÓÃµÄÒªÈûID ",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);		yhini->jtbh  = atoi(tttmp);
		GetPrivateProfileString("ÉèÖÃ", "ÅÝÐË·ÜÐÒÔËÐÒ¸£ÒªÇóµÈ¼¶ ",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);		yhini->pjy  = atoi(tttmp);
		GetPrivateProfileString("ÉèÖÃ", "ÅÝÔÚÏßÊ±¼äÒªÇóµÈ¼¶ ",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);		yhini->zxsj  = atoi(tttmp);
		...
		GetPrivateProfileString("ÉèÖÃ", "»úÐµÈÎÎñ¿ª¹Ø ",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);		yhini->jxrw  = atoi(tttmp);
...
//	USER *pUser = NULL;
	...
	for(i = 0; i < g_arDGArray.GetSize(); i++)//µç¹â°å
	{
		if(g_arDGArray[i]) 
		{
			delete g_arDGArray[i];
		}
	}
	g_arDGArray.RemoveAll();
