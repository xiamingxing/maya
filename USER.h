...
#define CLIENT_WALK_TICK	100//500      //×ßÂ·ËÙ¶È
#define CLIENT_RUN_TICK	    300//350      //ÅÜ²½ËÙ¶È
...
#define USE_ITEM_TIME		300			// ÎïÆ·Ê¹ÓÃ¼ä¸ôÊ±¼ä ÏÞÖÆ·è¿ñ¼ÓÑª
...
#define BIANSHEN_TIME		(3600 * 2 * 1000) //(3600 * 2 * 1000) 
...
//	int			m_iTradeSlotNum;						// ¾ÆÀÌÅÛÀ» Â÷·ÊÂ÷·Ê ¿Ã·Á³õÀ»¶§ ÇØ´ç °Å·¡ ½½·Ô¹øÈ£ 01_4_20
	...
	DWORD       m_dwLastBHTime;
    DWORD       m_dwLastSJTime;
...
protected:
	...
	void BaiJiHuangYuan(int Solt,int SoltDJ);///Èç¹ûÊÇ°Ù¼¶»¹Ô­
...
//	void EndFortressWar();
	...
	void shuxXING(int nUserID);//5111SAY
...
//	void CheckMemoryDB(char* strAccount, char* strUserId);
	...
	void RobLINGE(int dn);//ÐÞÁ¶
	BOOL CheckVip(int min);//NPC¼ì²â
	BOOL CheckVip2(int min);//NPC¼ì²â»áÔ±2
...
//	void NewGuild(TCHAR *pBuf);
	...
	void WriteUserShopLog(TCHAR *strContents);  
...
#define HUFA_DEA  1
	...
	void Diaoluo();
	void YJmzb();//Ò»¼üÂô×°±¸
	void Add4Pai(short storeID);
	void Add4PaiReq(TCHAR* pBuf);
	void  KuaiSuZuDui();
