...
//	GetLocalTime(&gTime);	
	...
	{												
		...
		if(abs(pUser->m_sLevel-m_byClassLevel) >o_yehuoini[0]->djxz)  //ÈËÎïÓë¹Ö²î¾à40¼¶ÎÞ·¨»ñµÃ¾­ÑéÏÞÖÆ´¦
			 continue;
		//if(pUser->m_dwShopPingDN > 3000 )  //105¼¶ºó²»ÄÜµÃµ½¾­Ñé£¬É±ËÀ¹ÖÎï¼¸¸öºó²»ÄÜµÃµ½¾­Ñé
		//	 continue;
...
/*	if(gTime.wDay > 7 && gTime.wDay < 14)					// 8ÀÏºÎÅÍ 13ÀÏ±îÁö	
	...
	{
*/
	if(TreeBaoLv ==TRUE || g_sanBaoLv ==TRUE)		nCount = 3;		//¸ù¾Ý»î¶¯ÊÇ·ñ±¬¶àÉÙ¼þ×°±¸
	else							nCount = 2;
...
//			GetLocalTime(&gTime);
	...
	*/
	...
	if(m_sPid>255)
		return;
	int tItemPer=g_DropItem[m_sPid][m_byColor].DropNovelity;                      //µôÎïÆ·¼¸ÂÊ
	int tLeechdomPer = g_DropItem[m_sPid][m_byColor].DropLeechdom+tItemPer;      //µôÀ¶¼¸ÂÊ
	int tItemN=g_DropItem[m_sPid][m_byColor].n;                                 //ÎïÆ·ÖÖÀàÊýÁ¿
	int i;
	//TRACE("ÊýÁ¿ %d\n",nCount);
	for(i = 0; i < nCount; i++)
	{
		...
		if(iRandom < tItemPer)
		{
			...
			if(iRandom < tItemHavePer){	
				...
				{
					CString strMsg;
					strMsg.Format("Íæ¼Ò¡º %s ¡» ´ò¹Ö»ñµÃ¡º ËÀÍöÊ¹Õß ¡»!", pUser->m_strUserID);//¹«¸æÌáÊ¾
					pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
				}
				if (iVal == 1093)
				{
					CString strMsg;
					strMsg.Format("Íæ¼Ò¡º %s ¡» ´ò¹Ö»ñµÃ¡º ±ù»ê ¡»!", pUser->m_strUserID);//¹«¸æÌáÊ¾
					pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
				}*/
...
/*	if(m_sEvent== 32000){
	...
	/*if(m_sEvent == NPC_EVENT_MOP )				// ÇöÀç ºòÇ²,Äý,½ºÄÝÇÇ¿À
	{
		iMagicTemp = 0; 
		iRareTemp = 4; 
	}
	...
	/*if(m_lKillUid >= 0)   
	{
		...
		if(pUser != NULL && pUser->m_state == STATE_GAMESTARTED)
		{
			...
			{
				iMagicTemp *= 7; //o_yehuoini[0]->xyl;  //ÐÒÔËÊ±µôÀ¶×°±¸
				iRareTemp *= 15; //o_yehuoini[0]->xyh;    //ÐÒÔËÊ±µô½ð×°±¸
				    
			}
			else if(pUser->m_dwMagicFindTime == 0 || pUser->m_isDoubleBAOLV == 0)
			{
				iMagicTemp *= 4; //o_yehuoini[0]->lan;  //Ã»ÓÐÐÒÔËÊ±µôÀ¶×°±¸
				iRareTemp *= 1; //o_yehuoini[0]->huang;    //Ã»ÓÐÐÒÔËÊ±µô½ð×°±¸
			}
...
int i = 0, j;
	...
	if(m_lKillUid >= 0)
	{
		...
		if(pUser != NULL && pUser->m_state == STATE_GAMESTARTED)
		{
			if(pUser->m_dwMagicFindTime > 0 || pUser->m_isDoubleBAOLV == TRUE || g_sanBaoLv == TRUE)	//ÐÒÔË ¾è¿î »òÕß·þÎñÆ÷Èý±¶¶¼Ö»±¬»Æ
			{	
				if(random <= 6500)		//5% »Æ½ð
				{
					if(random2 <= 1)		//3ÅÅ
					{
						nLoop = 3;
				...
				else if( random > 3500 && random <= 6500) //25% À¶É«
				{
					if(random2 <= 5000)		//1ÅÅ
					{
						nLoop = 1;
			...
			else									//²»ÊÇÐÒÔËÊ±
			{
				...
				else if( random > 400 && random <= 600) //25% À¶É«
				{
					if(random2 <= 5000)		//1ÅÅ
					{
						nLoop = 1;
	...
	if(m_ItemUserLevel <= 20)       iMagicCount = 205;		// Ñô¹âÉ¾³ýÀ¬»øÊôÐÔÖ÷ÒâµÄµØ·½
	...
	while(nLoop > i)										// ¹«Á¶°Ç 4°³¼Ó¼º°ªÀ» ±¸ÇÑ´Ù.
	{
		...
		{
			if(i == 0) continue;							
			else if(iType == RARE_ITEM && i <= 3) continue;	// Èç¹ûÊÇ½ð×°±¸.ÊôÐÔ±ØÐë4Ìõ 4ÅÅ <=3 ÊÇ4ÅÅ
			else { i++; continue; }
		}
		for(j = 0; j < 4; j++)
		{
			if(o_yehuoini[0]->chongdie == 0)
		  {
			...
			/*	int bianhao = pItem->sSid;
				...
				if ( tClass == 1 )//¼ì²âÀàÐÍÎªÎäÆ÷¸øÆÀ·Ö
				{
					//ÊôÐÔ 128 ËðÉË25     138ÃüÖÐ10    93¹¥»÷ËÙ¶È3    145ËùÓÐ¼¼Êõ2   113ÐË·Ü2   117·¨Êõ2   120´©´Ì2    125Á¬Éä2
					...
					//ÊôÐÔ  91ËðÉË20   94ÃüÖÐ8   72¹¥»÷ËÙ¶È2  
					...
					//ÊôÐÔ 64ËðÉË15  73ÃüÖÐ6  137ËùÓÐ¼¼ÊõµÈ¼¶1   49ÐË·Ü¼¼ÊõµÈ¼¶1    53·¨Êõ´óÊ¦µÈ¼¶1    56´©´Ì¼¼ÊõµÈ¼¶1   61Á¬Éä¼¼ÊõµÈ¼¶1
	...
	if ( tClass == 1 && pingfen >10 )//ÎïÆ·ÎªÎäÆ÷ ÆÀ·Ö´óÓÚ0 ÊýÖµ×Ô¼º¸Ä
	{
		CString strMsg;
		CString GoodsMagic;
		for(int i = 0; i < 4; i++)
		{
			if(pItem->tMagic[i] != 0)
			{
				...
				GoodsMagic+=",";
                                 ...
                                 /*ÆÀ·Ö[ %d ]*/                    /* ,pingfen*/
      /* strMsg.Format("Íæ¼Ò:¡º %s ¡» ´òµ½¡º %s ¡»ÊôÐÔ:", pUser->m_strUserID,g_arItemTable[bianhao2]->m_strName );//¹«¸æÌáÊ¾
		strMsg+=GoodsMagic;
		pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
	}
	else if ( tClass != 1 && pingfen > 10 )//ÎïÆ·Îª×°±¸ ÆÀ·Ö´óÓÚ0 ÊýÖµ×Ô¼º¸Ä
	{
		CString strMsg;
		CString GoodsMagic;
...
// 						}
				...
				////////////////////////////////////////////////////////////////////////ÔÂÉñÌ×
					...
					switch(iRandom){
						...
						case 7: tMagic=33;break;//»Ø±Ü5
						case 8: tMagic=14;break;//Ä§·¨ËðÉË5
						case 9: tMagic=6;break;//ËðÉË5
						case 10: tMagic=31;break;//ÃüÖÐ5
					    case 11: tMagic=137;break;//ËùÓÐ1
				...
				else if(iItemNum==1287){//Á¶ÓüÇò
					byte tMagic=0;
					iRandom = myrand(1, 8);
					switch(iRandom){
						case 1: tMagic=162;break;//15Á¦
						case 2: tMagic=156;break;//15ÌåÖÊ
						case 3: tMagic=158;break;//15Ãô½Ý
						case 4: tMagic=164;break;//15ÖÇ»Û
						case 5: tMagic=166;break;//15ÖÇÁ¦
						case 6: tMagic=190;break;//
						case 7: tMagic=193;break;//150/50¿¹Ñª
						case 8: tMagic=194;break;//50·À
						default: tMagic=0;break;
					}
					pMapItem->tIQ = MAGIC_ITEM;	
					pMapItem->tMagic[0] =tMagic;
				...
				else if(iItemNum== 1175) //¾§Æ¬
 				{
 					pMapItem->tIQ = MAGIC_ITEM;
 					pMapItem->tMagic[0] = 2;
 				}
 				else if(iItemNum == 1176) //¿â¶¡
 				{
 					pMapItem->tIQ = MAGIC_ITEM;
 					pMapItem->tMagic[0] = 3;
 				}
				else if(iItemNum == 1174) //ÏÉÉª
 				{
 					pMapItem->tIQ = MAGIC_ITEM;
 					pMapItem->tMagic[0] = 1;
 				}
				else if(iItemNum == 1177) //V3¸½¼þ
 				{
 					pMapItem->tIQ = MAGIC_ITEM;
 					pMapItem->tMagic[0] = 7;
 				}
...
*/					if(m_pGuardFortress) 
		...
		if( m_sEvent == NPC_EVENT_GREATE_MOP || m_sEvent== NPC_EVENT_MOP )		 //Èç¹û´òBOSS¹¥»÷Ö»ÓÐÆ½Ê±µÄ50%.
		{
...
go_result:
	...
	/*	////////////////////////////////////////////////////////////////////////////////////////////////////////////// É±BOSSÌáÐÑ
		if( m_sEvent == 31000 )
		{
			...
			{
				Dead();
			    strMsg.Format( "Íæ¼Ò¡º %s ¡»Õ½Ê¤BOSS¡º %s ¡» »ñµÃ´óÁ¿µÄ±¦±´!", pUser->m_strUserID,m_strName);
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
		}*/
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CNpc::NpcLive(COM *pCom)
{
	if(SetLive(pCom))
	{
        ...
        if(m_sEvent == 31000)
		{
			...
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ [ ÎÛÈ¾¶´3²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 49)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ [ ¿ó¶´4²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 400)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ ·ÆÀûÆÕ ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 93)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ ÈÌÕß¶´1²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 94)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ ÈÌÕß¶´2²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 36)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ µç³§3²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 416)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ ËÀÉñ¶´Ñ¨ ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 311)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ »ðÉñ¶´2²ã ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
			if(m_sCurZ == 315)
			{
				CString strMsg;
				strMsg.Format("ÁìÖ÷BOSS¡º %s ¡»ÔÚ  [ »ú¼×¹¤³§ ] Ë¢ÐÂÁË!", m_strName);//¹«¸æÌáÊ¾
				pCom->Announce((LPTSTR)(LPCTSTR)strMsg, SYSTEM_ANNOUNCE);
			}
		}
	}
	...
	if(tWeaponClass != 255)
	{
		for(i = tClass; i < tClass + SKILL_NUM; i++)	// IronSkill
		{
			...
			if(iSkillSid == SKILL_BACK_ATTACK)					// ¹Ý°Ý 2 index
			{
				...
				if(pUser->GetDistance(m_sCurX, m_sCurY, 1) == FALSE && pUser->m_dwFANTAnTime == 0 ) iCA = 0;//³¬¼¶·´µ¯
			...
			if(iSkillSid == SKILL_ABSOLUTE_DEFENSE)					// ¾ø¶Ô Defense
			{
			...
			if(iSkillSid == SKILL_DEFENSE_UP)					// ³¬¼¶ Defense
			{
...
//		pUser->SetFaNu(result);
		...
		else
		{
			...
			if(m_sPid == 204) //ºÚ°µËÀÉñ ±äÄ§·¨ÕÐ¹Ö
			{
				int iRandom = myrand(1, 10000);
				if(iRandom <= 3000)	
				{					
